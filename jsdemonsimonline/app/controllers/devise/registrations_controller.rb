class Devise::RegistrationsController < DeviseController
  ...
  def create
	...
	puts "putting signup params"
	puts sign_up_params
	puts resource
	connection = ActiveRecord::Base.connection
        user_email = sign_up_params[:email]
	cardslistcontents = File.read "cards.txt"
        st = ActiveRecord::Base.connection.raw_connection.prepare("UPDATE users SET cardslist = ? WHERE email = ?")
	st.execute(cardslistcontents, user_email)
	st.close
      else
  ...
  # GET /resource/cancel Forces the session data which is usually expired after sign in to be expired now. This is useful if the user wants to cancel oauth signing 
  # in/up in the middle of the process, removing all OAuth session data.
  def cancel
    expire_data_after_sign_in!
